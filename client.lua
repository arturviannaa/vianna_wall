...
vianna = Tunnel.getInterface("vianna_wall")
Config = {}
...
RegisterCommand("wall",function()
    ...
    if wall then
		drawNotification("~g~WALL ON.")
		vianna.reportwallLog("WALL ON")
		print("^2WALL ON!")
		PlaySoundFrontend(-1, "CONFIRM_BEEP", "HUD_MINI_GAME_SOUNDSET", 0)
	else
		drawNotification("~r~WALL OFF.")
		vianna.reportunwallLog("WALL OFF")
		print("^1WALL OFF!")
	end
end)
...
Citizen.CreateThread(
	function()
	    while true do
			...
			Citizen.Wait(ViannaStore)
		end
	end)
----------------------------------------------------------------------------------------------------------------------------------------
-- Config Das Linhas
----------------------------------------------------------------------------------------------------------------------------------------
RegisterCommand("lines",function()
	if not admin then
		return
	end
	...
	if lines then
		drawNotification("~g~LINHAS ON.")
		vianna.reportlinesLog("LINHAS ON")
		print("^2LINHAS ON!")
		PlaySoundFrontend(-1, "CONFIRM_BEEP", "HUD_MINI_GAME_SOUNDSET", 0)
	else
		drawNotification("~r~LINHAS OFF.")
		vianna.reportunlinesLog("LINHAS OFF")
		print("^1LINHAS OFF!")
	end
end)
----------------------------------------------------------------------------------------------------------------------------------------
...
Citizen.CreateThread(function()
	while true do
        local ViannaStore = 1000
	    for _, id in ipairs(GetActivePlayers()) do
			local pid = vianna.getId(GetPlayerServerId(id))
